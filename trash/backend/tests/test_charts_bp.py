...
try:
    ...
    from tools.data.chart_api import chart_api as charts_bp
...
def test_create_chart_happy_path(client, app, fake_chart_png):
    ...
    with patch("tools.data.chart_api.generate_chart", return_value=fake_chart_png):
        resp = client.post(
            f"/api/papers/{paper.id}/charts",
            json={
                "kind": "line",
...
def test_create_chart_propagates_value_error(client, app):
    ...
    with patch("tools.data.chart_api.generate_chart", side_effect=boom):
        resp = client.post(
            f"/api/papers/{paper.id}/charts",
