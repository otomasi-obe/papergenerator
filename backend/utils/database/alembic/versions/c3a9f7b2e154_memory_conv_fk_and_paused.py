...
def upgrade() -> None:
    ...
    with op.batch_alter_table("project_memory") as batch_op:
        batch_op.add_column(sa.Column("conversation_id", sa.String(length=20), nullable=True))
        ...
        batch_op.drop_constraint("uq_project_memory_paper_key", type_="unique")
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("project_memory") as batch_op:
        batch_op.create_unique_constraint("uq_project_memory_paper_key", ["paper_id", "key"])
        batch_op.drop_index("ix_project_memory_conversation_id")
        batch_op.drop_constraint("project_memory_conversation_id_fkey", type_="foreignkey")
