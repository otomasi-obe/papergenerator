...
def upgrade() -> None:
    with op.batch_alter_table("papers") as batch_op:
        batch_op.add_column(sa.Column("active_operation", sa.String(length=50), nullable=True))
        batch_op.add_column(
            sa.Column("active_operation_job_id", sa.String(length=50), nullable=True)
        )
        batch_op.add_column(sa.Column("active_operation_started_at", sa.DateTime(), nullable=True))
