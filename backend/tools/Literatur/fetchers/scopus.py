...
from ._elsevier import BASE, fetch_paginated, _parse_scopus_entry, elsevier_enabled
...
def search(client, query: str, limit: int = 25, filters: dict | None = None) -> Iterable[Paper]:
    ...
    entries = fetch_paginated(client, f"{BASE}/search/scopus", params, limit, 200)

    for entry in entries:
        if entry.get("error"):
            continue
        paper = _parse_scopus_entry(entry)
        if paper:
            yield paper
