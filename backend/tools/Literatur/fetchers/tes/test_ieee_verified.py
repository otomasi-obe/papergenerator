#!/usr/bin/env python3
"""Test IEEE fetcher - memastikan dapat data paper dan link download REAL
...
from tools.Literatur.fetchers import ieee
from tools.Literatur.http_client import get_client

def test_ieee():
    print("=" * 80)
    print("TESTING IEEE XPLORE FETCHER")
    print("=" * 80)
    
    api_key = os.getenv("IEEE_API_KEY")
    if not api_key:
        print("⚠️  IEEE_API_KEY tidak diset - fetcher akan di-skip")
        print("   Dapatkan API key gratis di: https://developer.ieee.org")
    ...
    papers = list(ieee.search(client, query, limit=limit))
    ...
    for i, paper in enumerate(papers, 1):
        ...
        print(f"   Article Number: {paper.source_id}")
        ...
        if paper.url:
            ...
            if any(x in paper.url for x in ["ieeexplore.ieee.org/document", "doi.org"]):
                print(f"   ✅ VALID LINK (IEEE Xplore atau DOI)")
                success_count += 1
            else:
