...
# Import asce module directly to avoid circular import
spec = importlib.util.spec_from_file_location(
    "asce",
    backend_path / "slr" / "fetchers" / "riset" / "asce.py"
)
asce = importlib.util.module_from_spec(spec)
spec.loader.exec_module(asce)
...
def test_asce_search_basic():
    ...
    try:
        ...
        # Perform search
        papers = list(asce.search(client, query, limit=limit))
...
def test_asce_search_with_filters():
    ...
    try:
        client = requests.Session()
        papers = list(asce.search(client, query, limit=limit, filters=filters))
...
def run_all_tests():
    ...
    results = {
        ...
        "Search with Filters": test_asce_search_with_filters(),
        "Metadata Validation": test_asce_paper_metadata(),
    }
