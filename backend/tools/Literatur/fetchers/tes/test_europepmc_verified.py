#!/usr/bin/env python3
"""Test Europe PMC fetcher - memastikan dapat data paper dan link download REAL"""
...
from tools.Literatur.fetchers import europepmc
...
def test_europepmc():
    ...
    papers = list(europepmc.search(client, query, limit=limit))
    ...
    for i, paper in enumerate(papers, 1):
        ...
        if paper.url:
            ...
            if any(x in paper.url for x in ["europepmc.org", "doi.org", "pdf"]):
                print(f"   ✅ VALID LINK")
                success_count += 1
            else:
