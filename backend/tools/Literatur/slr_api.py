...
def create_slr_job(paper_id: str):
    ...
    from tools.Literatur.slr import start_slr_job as new_start_slr_job

    job_id = new_start_slr_job(
        paper_id=paper_id,
        keyword=query,
        top_n=top_k,
        user_id=user_id,
...
def _wait_slr_jobs_inner(paper_id: str, user_id: int):
    ...
    try:
        while time.monotonic() < deadline:
            ...
            new_jobs_active = False
            try:
                orch_jobs = _get_orch_jobs_from_redis(paper_id, user_id)
                new_jobs_active = any(
                    j.get("status") in ("pending", "analyzing", "fetching", "ranking", "summarizing", "running")
                    for j in orch_jobs
                )
                if new_jobs_active:
                    ts = max(ts, time.time())
            ...
            if ts > after or new_jobs_active:
                jobs = (
                    db.session.query(SlrJob)
                    .options(defer(SlrJob.result))
                    .filter_by(paper_id=paper_id, user_id=user_id)
                ...
                # Add new orchestrator jobs (Redis-based, cross-worker)
                try:
                    orch_jobs2 = _get_orch_jobs_from_redis(paper_id, user_id)
                    job_dicts.extend(orch_jobs2)
                except Exception:
                    pass
...
def get_slr_job(job_id: str):
    ...
    if not job:
        # Check new orchestrator (in-memory/Redis jobs)
        from tools.Literatur.slr import get_slr_job as new_get_slr_job
        new_job = new_get_slr_job(job_id)
        if new_job:
...
def cancel_slr_job(job_id: str):
    ...
    if not job:
        ...
        from tools.Literatur.slr import _orchestrator as new_orch
        try:
            new_orch.cancel_job(job_id)
