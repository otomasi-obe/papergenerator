"""
IJEECSgen.py - Generator DOCX untuk jurnal IJEECS.
Menggunakan dokumen asli IJEECS.docx sebagai base template.
...
TEMPLATE_DOCX = BASE / "IJEECS.docx"
TEMPLATE_JSON = BASE / "_template.json"
OUTPUT_DOCX = BASE / "IJEECS_output.docx"
...
def clear_body(doc):
    ...
    Returns: (sectpr_paras, logo_elements)
      - sectpr_paras: list of paragraph elements with inline sectPr
      - logo_elements: list of (element, original_index) tuples for logo paragraphs/tables
    """
            ...
            if para_count <= 3:
                ...
                for run in child.findall(qn("w:r")):
                    if run.findall(qn("w:drawing")):
                        ...
                        break
                if has_image:
                    logo_elements.append((child, child_idx))
    ...
    for child in list(body):
        if child.tag == qn("w:sectPr"):
            continue
        is_logo = any(child is elem for elem, _ in logo_elements)
        if is_logo:
            continue
        if child.tag == qn("w:p"):
            ...
            if pPr is not None and pPr.find(qn("w:sectPr")) is not None:
                ...
                continue
        body.remove(child)
    return sectpr_paras, logo_elements
...
def generate():
    ...
    # --- Identify logo elements (preserve) and sectPr paras ---
    logo_elements = []
    sectpr_paras = []
    para_count = 0
    for child in body:
    ...
    # --- Remove ALL body content except logo elements and sectPr ---
    for child in list(body):
        if child.tag == qn("w:sectPr"):
            continue
        if child in logo_elements:
            continue
        if child in sectpr_paras:
            continue
        body.remove(child)

    # --- Build new content in a temp doc ---
    from docx import Document as _Document
    tmp_doc = _Document()
    tmp_body = tmp_doc._element.body

    # Generate title block
    add_title(tmp_doc, data)
    add_authors(tmp_doc, data)
    add_abstract(tmp_doc, data)
    add_keywords(tmp_doc, data)
    ...
    for i in range(1, 30):
        key = f"section{i}"
        if key in data:
            fig_counter, tbl_counter = process_section(tmp_doc, data[key], fig_counter, tbl_counter)

    # References
    add_references(tmp_doc, data)

    # --- Find insertion point in original body: after last logo element ---
    children_list = list(body)
    if logo_elements:
        last_logo_idx = max(list(body).index(le) for le in logo_elements)
    else:
        last_logo_idx = -1

    # --- Move all content from temp doc into original body, after logos ---
    # First, remove the temp doc's final sectPr (we'll use original's)
    tmp_sectpr = tmp_body.find(qn("w:sectPr"))
    if tmp_sectpr is not None:
        tmp_body.remove(tmp_sectpr)

    # Collect all children from temp body
    tmp_children = list(tmp_body)

    # Insert after last logo element
    insert_pos = last_logo_idx + 1
    for child in tmp_children:
        if insert_pos <= len(children_list):
