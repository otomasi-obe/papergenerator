"""
AMORIgen.py - Generator DOCX untuk jurnal AMORI.
Menggunakan dokumen asli AMORI.docx sebagai base template.
...
TEMPLATE_DOCX = BASE / "AMORI.docx"
TEMPLATE_JSON = BASE / "_template.json"
OUTPUT_DOCX = BASE / "AMORI_output.docx"
...
def set_run_font(run, font_name=None, size_pt=None, bold=None, italic=None, color=None):
    if font_name:
        ...
        rFonts.set(qn("w:cs"), CFG.get("font_cs", font_name))
        rFonts.set(qn("w:eastAsia"), CFG.get("font_eastAsia", font_name))
    if size_pt:
...
def remove_trailing_empty_sectpr_paras(doc, keep=None):
    """Remove trailing empty paragraphs that have inline sectPr, preserving ones in `keep` set."""
    body = doc._element.body
    to_remove = []
...
def _clean_formula_latex(text):
    """Minimal cleanup for formula LaTeX — just strip $ delimiters."""
    text = re.sub(r'\$([^$]+)\$', r'\1', text)
    # Fix HTML entities that might slip through
    text = text.replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>')
    text = text.replace('&quot;', '"').replace('&#39;', "'")
    # Fix \vec which causes malformed OMML in mathml2omml
    text = re.sub(r'\\vec\{', r'\\overrightarrow{', text)
    return text.strip()
...
def add_body_text(doc, text):
    ...
    # Auto-bold for bullet items (match template's bold bullets)
    bold_auto = cleaned.startswith("• ") or cleaned.startswith("□ ") or "Kontribusi spesifik" in cleaned or "Penelitian sebelumnya" in cleaned
    ...
    for seg in segments:
        if seg.startswith("**") and seg.endswith("**"):
            ...
            run = p.add_run(seg)
            set_run_font(run, CFG["font_body"], CFG["size_body"], bold=bold_auto)
...
def add_figure(doc, fig_data, fig_counter):
    ...
    if "Path" in fig_data and fig_data["Path"]:
        ...
        if img_p.exists():
            ...
            if alt.exists():
                image_path = alt
    if not image_path:
        url = fig_data.get("url", fig_data.get("image_url", ""))
    ...
    if image_path:
        ...
        set_para_spacing(p_img, before_pt=14, after_pt=8)
        try:
        ...
        if img_w_in > max_w_in:
            ...
            img_w_in *= scale
            img_h_in *= scale
        dw = int(img_w_in * 914400)
        dh = int(img_h_in * 914400)
        try:
            run = p_img.add_run()
            run.add_picture(str(image_path), width=Emu(dw), height=Emu(dh))
        except Exception:
            run = p_img.add_run("[Image could not be embedded]")
        ...
        run = p_img.add_run(f"[Figure {fig_no}: {title}]")
        set_run_font(run, CFG["font_body"], CFG["size_body"], italic=True, color=(0xFF, 0x00, 0x00))

    # Caption
    p_cap = doc.add_paragraph()
    p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(p_cap, before_pt=4, after_pt=14)
...
def add_table_element(doc, tbl_data, tbl_counter):
    ...
    set_para_spacing(p_title, before_pt=10, after_pt=6)
    ...
    if rows:
        ...
        set_para_spacing(p_spacer, before_pt=6, after_pt=8)
...
def add_formula(doc, formula_data):
    ...
    add_omml_formula(doc, latex, number, CFG, before_pt=6, after_pt=6,
                     alignment="center", font_body=CFG.get("font_body", "Times New Roman"),
                     size_body=CFG.get("size_body", 10), max_width_cm=max_fw,
                     font_cs=CFG.get("font_cs"), font_eastAsia=CFG.get("font_eastAsia"))
...
def generate():
    ...
    # Insert after body_insert_before (not before)
    if body_insert_before is not None and sectpr_paras:
            ...
            for child in list(body):
                ...
                try:
                    ...
                    pass
            # Insert AFTER body_insert_before
            for el in reversed(new_elements):
                body.remove(el)
                body.insert(list(body).index(body_insert_before) + 1, el)

    # Remove trailing empty sectPr paragraphs (fixes blank pages)
    remove_trailing_empty_sectpr_paras(doc, keep=set(sectpr_paras) if sectpr_paras else None)
