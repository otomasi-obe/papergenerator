"""
DJLITgen.py - Generator DOCX untuk jurnal DJLIT.
Menggunakan dokumen asli DJLIT.docx sebagai base template.
...
def set_para_spacing(para, before_pt=None, after_pt=None, line_tw=None, line_spacing=1.0):
    """Set paragraph spacing. Default line_spacing=1.0 (single spacing) for DJLIT."""
...
def set_table_borders(table, style="three_line"):
    # Skip border setting if style is "skip" (preserve original template structure)
    if style == "skip":
        return
...
def remove_trailing_empty_sectpr_paras(doc):
    ...
    Moves sectPr to last content paragraph to preserve section count."""
    body = doc._element.body
    ...
    for child in reversed(list(body)):
        ...
        if child.tag == qn("w:p"):
            ...
            if has_content:
                # Found content paragraph - this is our anchor
                last_content_para = child
                break
            elif has_sectpr:
                # Empty paragraph with sectPr - mark as trailing
                trailing_empty_sectprs.append(child)
            # Continue scanning for more trailing empty sectPr
        elif child.tag == qn("w:tbl"):
            # Table is content - stop here
            break

    # Move sectPr from trailing empty paragraphs to last content paragraph
    if last_content_para is not None and trailing_empty_sectprs:
        # Get or create pPr for last content paragraph
        pPr = last_content_para.find(qn("w:pPr"))
        if pPr is None:
            pPr = OxmlElement("w:pPr")
            last_content_para.insert(0, pPr)

        # Move sectPr from first trailing empty paragraph to last content paragraph
        # (only move one to preserve section structure)
        first_trailing = trailing_empty_sectprs[-1]  # Last in list = first encountered
    ...
    if headers:
        for j, h in enumerate(headers):
            if j < n_cols:
                ...
                cell.text = ""
                p = cell.paragraphs[0]
                set_para_spacing(p, before_pt=0, after_pt=0, line_spacing=1.0)
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
