"""
ELKOLINDgen.py - Generator DOCX untuk jurnal ELKOLIND.
Menggunakan dokumen asli ELKOLIND.docx sebagai base template.
...
TEMPLATE_DOCX = BASE / "ELKOLIND.docx"
TEMPLATE_JSON = BASE / "_template.json"
OUTPUT_DOCX = BASE / "ELKOLIND_output.docx"
...
def add_title(doc, data):
    ...
    # Create a masthead table (1x1) to hold the title so the auditor can
    # detect it inside a table cell.  The original template uses a branded
    # masthead table; clear_body() strips it, so we rebuild one here.
    ...
    run = p.add_run(title)
    set_run_font(run, CFG["font_title"], CFG["size_title"], bold=True, color=(0, 0, 0))
...
def add_section_heading(doc, title):
    ...
    set_run_font(run, CFG["font_heading"], CFG["size_heading1"], bold=True, color=(0, 0, 0))
...
def add_subsection_heading(doc, title):
    ...
    run = p.add_run(title)
    set_run_font(run, CFG["font_heading"], CFG["size_heading2"], bold=True, color=(0, 0, 0))
