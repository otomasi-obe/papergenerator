...
def _validate_paper_shape(paper: dict) -> dict:
    ...
    if not isinstance(refs, list):
        issues.append(f"references is not a list (got {type(refs).__name__})")
    elif len(refs) < 5:
        issues.append(f"references has {len(refs)} entries (suspiciously low — should have >=5)")
    # 5-14 refs: allow — may be user-provided exact N refs (EXACT COUNT enforced in prompt)
    # >= 15 refs: always fine
...
# 1. PaperRiset/eks/paperfull/prompt/         (relative to single.py here)
...
def _load_system_prompt(
    ...
    """Concatenate prompt + humanize + optional style/topic guides (language-aware).
    ...
    prompt_path = _resolve_prompt_file("prompt.txt")
    if language == "en":
    ...
    parts: list[str] = [lang_instruction + prompt_path.read_text(encoding="utf-8")]

    try:
        humanize_path = _resolve_prompt_file("humanize.txt")
        parts.append(humanize_path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        pass

    if style:
        try:
            style_path = _resolve_prompt_file(f"style/{style}.txt")
            parts.append(
    ...
    if topic:
        try:
            topic_path = _resolve_prompt_file(f"topic/{topic}.txt")
            parts.append(
                f"## TOPIC GUIDE — {topic}\n"
    ...
    if review_mode:
        try:
            review_path = _resolve_prompt_file("topic/review.txt")
            parts.append(
                "## TOPIC GUIDE (PAPER REVIEW MODE)\n"
                + review_path.read_text(encoding="utf-8")
            )
            log.info("[_load_system_prompt] Injected review topic guide")
        except FileNotFoundError:
            log.warning("[_load_system_prompt] topic/review.txt not found")
...
def _load_chat_history(conv_id: str | None, limit: int = 10) -> str:
    ...
    if not conv_id:
        return ""
    try:
        from utils.database.models import ChatMessage, db
...
def _load_attached_files(paper_id: str | None, limit: int = 50, char_cap: int = 5000) -> str:
    """Top `limit` PaperFile rows; each extracted_text is truncated to char_cap."""
    if not paper_id:
        return ""
    try:
        from utils.database.models import PaperFile, db
...
def _normalize_paper_shape(raw: dict) -> dict:
    ...
    if isinstance(refs, dict):
        if isinstance(refs.get("items"), list):
            ...
            # Dict of refs: extract only values that look like actual references
            # (dicts with title/authors/year or strings that look like citations)
            refs = [
                v for k, v in refs.items()
                if k.lower() not in ("title", "_catatan", "_aturan_referensi", "count", "source", "items")
                and isinstance(v, dict)
                and (v.get("title") or v.get("authors") or v.get("year"))
            ]
    if not isinstance(refs, list):
        refs = []
    ...
    *,
    ...
    Args:
        ...
        paper_id: Optional paper ID — used to load up to 50 ``LiteratureItem``
            ...
            (each extracted_text capped at 3000 chars).
    ...
    system_prompt = _load_system_prompt(
        style=style,
        topic=topic,
        review_mode=_detect_paper_review_mode(judul, custom_prompt),
        language=language,
    )

    # ── @slr tag handling (symmetric with chat) ──────────────────────────────
    # User can write "@slr" in the prompt to FORCE-inject pinned SLR literature.
    # The tag is stripped before the prompt reaches the AI (no literal leak).
    slr_tag_forced = False
    if custom_prompt and "@slr" in custom_prompt.lower():
        slr_tag_forced = True
        custom_prompt = re.sub(r"@slr\b", "", custom_prompt, flags=re.IGNORECASE).strip()
    ...
    if not has_lit_in_custom:
        # For paper review mode: load up to 100 literature items (all SLR papers)
        is_review = _detect_paper_review_mode(judul, custom_prompt)
        lit_limit = 100 if is_review else 50
        lit_block = _load_literature(paper_id, limit=lit_limit)
        if lit_block:
            user_parts.append(lit_block)
            log.info("[generate_paper_json_single] Loaded %d literature items (review_mode=%s)", 
                     lit_block.count("[L"), is_review)
    ...
    # @slr tag atau review mode → naikkan limit (lebih banyak referensi).
    if paper_id and user_id:
        try:
            from tools.Literatur.literature_helpers import get_pinned_literature
            pinned_max = 50 if (slr_tag_forced or _detect_paper_review_mode(judul, custom_prompt)) else 10
            pinned_block = get_pinned_literature(paper_id, user_id, max_items=pinned_max)
            if pinned_block:
                user_parts.append(f"## SLR References (Pinned by user)\n{pinned_block}")
                log.info("[generate_paper_json_single] Injected pinned SLR literature for paper=%s (max=%d, forced=%s)", paper_id, pinned_max, slr_tag_forced)
        except Exception as e:
    ...
    if not has_files_in_custom:
        files_block = _load_attached_files(paper_id, limit=50, char_cap=5000)
        if files_block:
