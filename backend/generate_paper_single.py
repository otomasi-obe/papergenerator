...
# shape + validation) under backend/log/generator/<paper_id>/<job_id>/. This
# lets us SEE what's going wrong before waiting 5–10 minutes for a partial
# paper to surface in the editor.
import datetime as _dt
import json as _json

_GEN_LOG_ROOT = Path(__file__).parent / "log" / "generator"
...
def _gen_log_dir(paper_id, job_id) -> Path | None:
    try:
        d = _GEN_LOG_ROOT / _safe_seg(paper_id, "_no-paper") / _safe_seg(job_id, "_no-job")
...
def generate_paper_json_single(
    ...
    *,
    ...
    gen_dir = _gen_log_dir(paper_id, job_id)
