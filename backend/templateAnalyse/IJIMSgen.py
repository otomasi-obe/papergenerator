...
TEMPLATE_PATH = BASE / "IJIMS.doc"
JSON_PATH = BASE / "_template.json"
OUTPUT_PATH = BASE / "output" / "IJIMS_output.docx"

CONVERTED_TEMPLATE = Path("/tmp/kilo/IJIMS.docx")
...
def setup_footer_odd(section, data):
    ...
    set_run_font(run, "Goudy Old Style", 10)
...
def add_authors(doc, data):
    ...
    for author in authors:
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_paragraph_spacing(p, before=0, after=0, line=240, line_rule="auto")
        run = p.add_run(author.get("name", "Author Name"))
        set_run_font(run, "Goudy Old Style", 14, bold=True)

    for author in authors:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_paragraph_spacing(p, before=0, after=0, line=240, line_rule="auto")
        run = p.add_run(author.get("affiliation", "Institution"))
        set_run_font(run, "Goudy Old Style", 11.5)

    for author in authors:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_paragraph_spacing(p, before=0, after=0, line=240, line_rule="auto")
        run = p.add_run(author.get("email", "email@mail.ac.id"))
        set_run_font(run, "Goudy Old Style", 11.5)
...
def add_abstract(doc, data):
    ...
    set_run_font(run, "Goudy Old Style", 10.5)


def add_keywords(doc, data):
    add_empty_line(doc, "Goudy Old Style", 10)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    set_paragraph_spacing(p, before=0, after=0, line=240, line_rule="auto")
    run_label = p.add_run("Keywords: ")
    set_run_font(run_label, "Goudy Old Style", 10, bold=True)

    keywords = data.get("keywords", ["keyword one", "keyword two", "keyword three"])
    kw_text = "; ".join(keywords)
    run_kw = p.add_run(kw_text)
    set_run_font(run_kw, "Goudy Old Style", 10, italic=True)
...
def add_subsection_heading(doc, title):
    ...
    run = p.add_run(title)
    set_run_font(run, "Goudy Old Style", 10.5, bold=True)
    return p
...
def add_body_text(doc, text):
    ...
    run = p.add_run(text)
    set_run_font(run, "Goudy Old Style", 11)
    return p
...
def add_figure(doc, fig_data):
    ...
    if img_full_path and img_full_path.exists():
        ...
        run = p_img.add_run("[Image placeholder]")
        set_run_font(run, "Goudy Old Style", 10, italic=True)
    ...
    set_run_font(run_title, "Goudy Old Style", 10)
...
def add_formula(doc, formula_data):
    ...
    set_run_font(run_num, "Goudy Old Style", 11)
...
def add_table(doc, table_data):
    ...
    for i, h in enumerate(headers):
        ...
        set_paragraph_spacing(p, before=0, after=0, line=240, line_rule="auto")
        run = p.add_run(h)
        set_run_font(run, "Goudy Old Style", 9, bold=True)
    ...
    for ri, row in enumerate(rows):
        for ci, cell_text in enumerate(row):
            ...
            set_run_font(run, "Goudy Old Style", 9, bold=is_bold)
...
def generate():
    ...
    if CONVERTED_TEMPLATE.exists():
        doc = Document(str(CONVERTED_TEMPLATE))
