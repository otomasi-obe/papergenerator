...
"""
import json, copy, zipfile, shutil, os, re
...
def set_paragraph_format(para, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=0, space_after=0,
                         line_spacing=1.0, first_line_indent=None, left_indent=None, hanging_indent=None):
...
def add_title(doc, data):
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.CENTER, space_before=0, space_after=0, line_spacing=1.0)
...
def add_abstract(doc, data):
    ...
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=0, space_after=0, line_spacing=1.0)
...
def add_section_heading(doc, title, space_before=3, space_after=3):
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=space_before, space_after=space_after, line_spacing=1.0)
    run = p.add_run(title)
    set_run_font(run, bold=True)
    return p
...
def add_body_text(doc, text, first_line_indent=720):
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=0, space_after=0,
                         line_spacing=1.0, first_line_indent=first_line_indent)
    run = p.add_run(text)
    set_run_font(run)
    return p
...
def add_figure(doc, fig_data, space_before=3, space_after=3):
    ...
    set_paragraph_format(p_img, alignment=WD_ALIGN_PARAGRAPH.CENTER, space_before=space_before, space_after=0, line_spacing=1.0)
    ...
    if img_path.exists():
        ...
        run = p_img.add_run("[Image not found: " + fig_data.get("Path", "") + "]")
        set_run_font(run, italic=True)

    p_cap = doc.add_paragraph()
    set_paragraph_format(p_cap, alignment=WD_ALIGN_PARAGRAPH.CENTER, space_before=0, space_after=space_after, line_spacing=1.0)
...
def add_formula(doc, formula_data, space_before=3, space_after=3):
    ...
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.CENTER, space_before=space_before, space_after=space_after, line_spacing=1.0)
...
def add_table(doc, table_data, space_before=6, space_after=3):
    ...
    set_paragraph_format(p_title, alignment=WD_ALIGN_PARAGRAPH.CENTER, space_before=space_before, space_after=space_after, line_spacing=1.0)
...
def add_references(doc, data):
    ...
    add_empty_para(doc)
    p = doc.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=3, space_after=3, line_spacing=1.0)
    ...
    for ref in refs_content:
        p = doc.add_paragraph()
        set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY, space_before=0, space_after=0,
                             line_spacing=1.0, left_indent=567, hanging_indent=567)
        ...
        for part in parts:
            if part.startswith('"') and part.endswith('"'):
                ...
                title_match = re.search(r',\s*([^,]+(?:,\s*(?:ed\.|vol\.|no\.)[^,]*)*)\s*,', part)
                run = p.add_run(part)
                set_run_font(run, color="000000")
...
def process_section(doc, section_data, section_num, is_top_level=True):
    ...
    sub_keys = sorted([k for k in section_data.keys() if k.startswith(f"section{section_num}") and k != "title" and k != "content"])
    for sk in sub_keys:
