#!/usr/bin/env python3
"""Test ScienceDirect fetcher - memastikan dapat data paper dan link download REAL
...
from slr.fetchers import sciencedirect
from slr.http_client import get_client

def test_sciencedirect():
    print("=" * 80)
    print("TESTING SCIENCEDIRECT FETCHER")
    print("=" * 80)
    
    api_key = os.getenv("ELSEVIER_API_KEY")
    if not api_key:
        print("⚠️  ELSEVIER_API_KEY tidak diset - fetcher akan di-skip")
        print("   Dapatkan API key di: https://dev.elsevier.com")
    ...
    papers = list(sciencedirect.search(client, query, limit=limit))
    ...
    for i, paper in enumerate(papers, 1):
        ...
        print(f"   PII/DOI: {paper.source_id}")
        ...
        if paper.url:
            ...
            if any(x in paper.url for x in ["sciencedirect.com", "doi.org"]):
                print(f"   ✅ VALID LINK")
                success_count += 1
            else:
