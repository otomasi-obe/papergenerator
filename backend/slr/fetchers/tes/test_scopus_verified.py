#!/usr/bin/env python3
"""Test Scopus fetcher - memastikan dapat data paper dan link download REAL
...
from slr.fetchers import scopus
from slr.http_client import get_client

def test_scopus():
    print("=" * 80)
    print("TESTING SCOPUS FETCHER")
    print("=" * 80)
    ...
    papers = list(scopus.search(client, query, limit=limit))
    ...
    for i, paper in enumerate(papers, 1):
        ...
        print(f"   Scopus ID: {paper.source_id}")
        ...
        if paper.url:
            ...
            if any(x in paper.url for x in ["scopus.com", "doi.org", "elsevier.com"]):
                print(f"   ✅ VALID LINK")
                success_count += 1
            else:
