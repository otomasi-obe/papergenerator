...
from core.chart_generator import ChartSpec, generate_chart, parse_data_file
from database.models import Paper, PaperImage, db
from paper_generation.utils import safe_paper_dir, PAPER_ID_RE
...
@charts_bp.route("/api/papers/<paper_id>/charts", methods=["GET"])
@jwt_required()
...
@charts_bp.route("/api/papers/<paper_id>/charts/<int:chart_id>", methods=["GET"])
@jwt_required()
...
@charts_bp.route("/api/papers/<paper_id>/charts/<int:chart_id>", methods=["PUT"])
@jwt_required()
...
@charts_bp.route("/api/papers/<paper_id>/charts/<int:chart_id>", methods=["DELETE"])
@jwt_required()
...
@charts_bp.route("/api/papers/<paper_id>/charts/upload-data", methods=["POST"])
@jwt_required()
def upload_chart_data(paper_id: str):
    """Upload CSV/Excel file and parse it for chart generation.
    
    Returns parsed data structure that can be used to create charts.
    """
    if not PAPER_ID_RE.match(paper_id):
        return _err("Invalid paper id", "BAD_REQUEST", 400)
