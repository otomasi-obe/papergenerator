...
class TestImageInterceptTimeout:
    ...
    def test_intercept_timeout_scales_with_generation(self):
        ...
        assert (
            "intercept_timeout = generate_timeout_s + 30" in code
        ), "Code should have dynamic intercept timeout"
        assert (
            "deadline = time.time() + intercept_timeout" in code
...
class TestCompressionFailureHandling:
    ...
    def test_compression_failure_deletes_file(self):
        ...
        assert (
            "if not compress_image(out_path, max_size_mb=1.0):" in code
        ), "Should check compression return value"
        assert (
            "raise RuntimeError" in code and "compression failed" in code.lower()
    ...
    def test_compression_success_logs_size(self, mock_compress):
        ...
        assert "out_path.stat().st_size // 1024" in code, "Should log file size in KB"
...
class TestUIOperationTimeouts:
    ...
    def test_open_image_tool_has_timeout(self):
        ...
        assert (
            "def _open_image_tool(page, *, timeout_s: int = 30)" in code
        ), "_open_image_tool should have timeout parameter"
        assert (
    ...
    def test_send_prompt_has_timeout(self):
        ...
        assert (
            "def _send_prompt(page, prompt: str, *, timeout_s: int = 30)" in code
        ), "_send_prompt should have timeout parameter"
        assert (
            "box.click(timeout=timeout_s * 1000)" in code
...
class TestModelDocumentation:
    ...
    def test_status_field_documents_cancelled(self):
        ...
        assert (
            "queued|running|done|error|cancelled" in code
    ...
    def test_worker_field_has_description(self):
        ...
        assert (
            "which account picked it up (account1..account4)" in code
...
class TestErrorMessages:
    ...
    def test_cookie_error_message_is_clear(self):
        ...
        assert (
            "Cookie penting" in code and "tidak muncul" in code
        ), "Should explain which cookie is missing"
        assert "login ulang" in code or "refresh" in code, "Should suggest how to fix"
    ...
    def test_intercept_error_message_is_clear(self):
        ...
        assert (
            "network issue" in code or "API berubah" in code or "rate limit" in code
    ...
    def test_compression_error_message_is_clear(self):
        ...
        assert "Original size:" in code, "Should show original file size"
