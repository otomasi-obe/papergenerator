...
def test_beginner_user_workflow():
    ...
    assert agent.verify_response_contains("bidang", response1) or \
           agent.verify_response_contains("topik", response1) or \
           agent.verify_response_contains("minat", response1), \
    ...
    assert agent.verify_response_contains("spesifik", response2) or \
           agent.verify_response_contains("aplikasi", response2) or \
           agent.verify_response_contains("masalah", response2), \
    ...
    assert agent.verify_response_contains("judul", response3) or \
           agent.verify_response_contains("struktur", response3) or \
           agent.verify_response_contains("outline", response3), \
...
def test_intermediate_user_workflow():
    ...
    response1 = agent.wait_for_ai_response(timeout=30)
    assert agent.verify_response_contains("arsitektur", response1) or \
           agent.verify_response_contains("metodologi", response1) or \
           agent.verify_response_contains("dataset", response1), \
    ...
    response2 = agent.wait_for_ai_response(timeout=30)
    assert agent.verify_response_contains("evaluasi", response2) or \
           agent.verify_response_contains("metrik", response2) or \
           agent.verify_response_contains("hasil", response2), \
...
Tolong generate paper lengkap dengan struktur IEEE format."""
    ...
    response1 = agent.wait_for_ai_response(timeout=30)
    assert agent.verify_response_contains("lengkap", response1) or \
           agent.verify_response_contains("generate", response1) or \
           agent.verify_response_contains("membuat", response1), \
...
def test_hybrid_literature_workflow():
    ...
    assert agent.verify_response_contains("mencari", response1) or \
           agent.verify_response_contains("paper", response1) or \
           agent.verify_response_contains("literatur", response1), \
           "AI should acknowledge search request"
    
    # Step 6: Wait for search results
    response2 = agent.wait_for_ai_response(timeout=60)
    assert agent.verify_response_contains("menemukan", response2) or \
           agent.verify_response_contains("hasil", response2) or \
           agent.verify_response_contains("paper", response2), \
...
def test_error_handling():
    ...
    agent.send_chat_message("")
    response = agent.wait_for_ai_response(timeout=10)
    # Should handle gracefully, not crash
    
    # Test 2: Very long message
    long_message = "A" * 10000
    agent.send_chat_message(long_message)
    response = agent.wait_for_ai_response(timeout=30)
