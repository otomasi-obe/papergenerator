...
CHECKER_SCRIPT = Path("/home/sirobo/ANNABIL/Alur Checking/auto_checker.py")

# Pipeline fixes
sys.path.insert(0, str(BACKEND))
from main import _normalize_formulas, _strip_ref_numbering
...
def run_generate(journal_code, json_path, output_path):
    ...
    try:
        # Load JSON and apply pipeline fixes
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8') as tmp:
            json.dump(paper, tmp, ensure_ascii=False, indent=2)
            tmp_json_path = tmp.name
        
        try:
            canonical, builder = _get_builder(journal_code)
            result = builder(tmp_json_path, str(output_path))
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(CHECKER_SCRIPT),
             ...
             "--json", str(TEMPLATE_JSON.resolve())],
