...
def add_figure(doc, fig_data):
    ...
    set_paragraph_spacing(p_img, before=60, after=60,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
    ...
    set_paragraph_spacing(p_cap, before=60, after=120,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
...
def add_formula(doc, formula_data):
    ...
    set_paragraph_spacing(p_num, before=0, after=60,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
...
def add_table(doc, table_data):
    ...
    set_paragraph_spacing(p_title, before=120, after=60,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
    ...
    set_paragraph_spacing(p_after, before=60, after=0,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
...
def process_section(doc, section_key, section_data, level=1, parent_num=None):
    ...
    next_parent = f"{parent_num}.{own_n}" if (level > 1 and parent_num and own_n) else (own_n if level == 1 else parent_num)
    for sk in sub_keys:
...
def add_references(doc, data):
    ...
    set_paragraph_spacing(p_title, before=60, after=60,
                          line=CFG["line_spacing_tw"],
                          line_rule=CFG["line_spacing_rule"])
    ...
    for ref in refs_content:
        ...
        set_paragraph_spacing(p, before=0, after=0,
                              line=CFG["line_spacing_tw"],
                              line_rule=CFG["line_spacing_rule"])
        set_paragraph_indent(p, left=CFG["ref_hanging_indent_tw"],
...
def generate():
    ...
    section_keys = [k for k in data.keys()
                    if k.startswith("section") and isinstance(data[k], dict)]
