...
def _add_run(paragraph, text, *, size_pt=None, bold=None, italic=None, font=None, font_name=None, color=None, vert_align=None, **_):
    if font_name is not None and font is None: font = font_name
...
def add_title(doc, data):
    ...
    _set_para_format(p, align="center",
                     sp_before_tw=0, sp_after_tw=480,
                     line_tw=CFG["line_single_tw"], line_rule="auto",
                     ind_left_tw=-720, ind_first_tw=0,
                     keep_next=True)
    _add_run(p, title,
...
def add_abstract(doc, data):
    ...
    _add_run(label, "ABSTRACT",
             font_name=CFG["font_body"],
             size_pt=CFG["size_abstract_label"],
             bold=True)
...
def add_keywords(doc, data):
    ...
    _add_run(p, "Keywords",
             font_name=CFG["font_body"],
             size_pt=CFG["size_keywords_label"],
             bold=True)
    _add_run(p, ": ",
             font_name=CFG["font_body"],
             size_pt=CFG["size_keywords_label"],
...
def add_body_text(doc, text, first_paragraph=False):
    ...
    _set_para_format(
        p, align="both",
        sp_before_tw=0, sp_after_tw=240,
        line_tw=CFG["line_body_tw"], line_rule="auto",
...
def add_figure_placeholder(doc, fig):
    ...
    prompt = (fig.get("Prompt") or
              "Generate a clean academic figure suitable for a journal paper."
              ).strip()
...
def add_table_block(doc, table_data):
    ...
    for i, h in enumerate(headers):
        ...
        _set_para_format(p, align="center",
                         ...
                         line_tw=CFG["line_single_tw"], line_rule="auto")
        _add_run(p, clean_inline_text(str(h)),
                 font_name=CFG["font_heading"],
...
def add_references(doc, data):
    ...
    for ref in items:
        ...
        _set_para_format(rp, align="both",
                         sp_before_tw=0, sp_after_tw=120,
                         line_tw=CFG["line_body_tw"], line_rule="auto",
...
def add_acknowledgement(doc, text=None):
    ...
    _set_para_format(p_title, align="left",
                     ...
                     line_tw=CFG["line_single_tw"], line_rule="auto",
                     keep_next=True)
    _add_run(p_title, "ACKNOWLEDGEMENTS",
             font_name=CFG["font_heading"],
             size_pt=CFG["size_heading1"],
             bold=True)
    ...
    _add_run(p, clean_inline_text(text or
             "Authors may also include the acknowledgement section."),
             font_name=CFG["font_body"],
             size_pt=CFG["size_body"])
