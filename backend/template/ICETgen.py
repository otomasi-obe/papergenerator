...
ROMAN_UPPER_TBL = ["", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX",
                   "X", "XI", "XII", "XIII", "XIV", "XV", "XVI", "XVII", "XVIII", "XIX", "XX"]
...
def set_para_spacing(p, before_pt=None, after_pt=None, line_pt=None,
                     alignment=None, left_tw=None, right_tw=None,
                     first_line_tw=None, hanging_tw=None, keep_next=False):
...
def make_sectpr(top_tw, bottom_tw, left_tw, right_tw, cols=1,
                col_space_tw=720, sect_type=None, title_pg=False,
...
def insert_section_break(doc, top_tw, bottom_tw, left_tw, right_tw,
                         cols, col_space_tw=720, title_pg=False,
                         footer_first_rid=None):
    """Sisipkan paragraf kosong dengan sectPr → continuous section break."""
    p = doc.add_paragraph()
    pPr = p._element.get_or_add_pPr()
    pPr.append(make_sectpr(top_tw, bottom_tw, left_tw, right_tw,
                           cols=cols, col_space_tw=col_space_tw,
                           sect_type="continuous", title_pg=title_pg,
...
def set_final_sectpr(doc):
    ...
    body.append(make_sectpr(CFG["body_top_tw"], CFG["body_bottom_tw"],
                            CFG["title_left_tw"], CFG["title_right_tw"],
                            cols=1, col_space_tw=CFG["col_space_tw"]))
...
def clear_body(doc):
    ...
    if sp is None:
        body.append(make_sectpr(CFG["title_top_tw"], CFG["title_bottom_tw"],
                                CFG["title_left_tw"], CFG["title_right_tw"],
                                cols=1))
...
def add_runs_with_inline(paragraph, text, base_font, base_size,
                         base_bold=False, base_italic=False, base_color=None):
    pos = 0
    text = text.replace(" ", " ")
    for m in _INLINE_RE.finditer(text):
        ...
        if m.group(1) is not None:
            ...
            add_run(paragraph, m.group(4), name=base_font, size_pt=base_size,
...
def add_author_notes(doc, data):
    ...
    for note in notes:
        ...
        add_run(p, note, name=CFG["font_serif"],
                size_pt=8.0, italic=True, color=(0, 0, 0))
...
def add_authors(doc, data):
    ...
    remove_numpr(name_p)
    set_para_spacing(name_p, before_pt=18, after_pt=2,
                     alignment=WD_ALIGN_PARAGRAPH.CENTER)
    for i, a in enumerate(authors):
        if i > 0:
            sep = ", and " if i == len(authors) - 1 else ", "
            add_run(name_p, sep, name=CFG["font_serif"],
                    size_pt=CFG["size_author"], color=(0, 0, 0))
        add_run(name_p, a.get("name", "Author Name"),
                name=CFG["font_serif"], size_pt=CFG["size_author"], color=(0, 0, 0))
        add_run(name_p, str(i + 1), name=CFG["font_serif"],
                size_pt=CFG["size_author"] * 0.7,
                superscript=True, color=(0, 0, 0))

    # Affiliations: tiap author punya satu paragraf Affiliation
    for i, a in enumerate(authors):
        ap = _new_para(doc, "Affiliation")
        remove_numpr(ap)
        set_para_spacing(ap, before_pt=2, after_pt=0,
                         alignment=WD_ALIGN_PARAGRAPH.CENTER)
        add_run(ap, str(i + 1), name=CFG["font_serif"],
                size_pt=CFG["size_affil"] * 0.75, superscript=True, color=(0, 0, 0))
        affil = a.get("affiliation", "Department, University")
        loc = a.get("location", "")
        full = (affil + (", " + loc if loc else ""))
        add_run(ap, " " + full, name=CFG["font_serif"],
                size_pt=CFG["size_affil"], italic=True, color=(0, 0, 0))
        if a.get("email"):
            ap2 = _new_para(doc, "Affiliation")
            remove_numpr(ap2)
            set_para_spacing(ap2, before_pt=0, after_pt=2,
                             alignment=WD_ALIGN_PARAGRAPH.CENTER)
            add_run(ap2, a["email"], name=CFG["font_serif"],
                    size_pt=CFG["size_affil"], color=(0, 0, 0))
...
def add_abstract(doc, data):
    ...
    set_para_spacing(p, before_pt=4, after_pt=8,
                     ...
                     first_line_tw=272)
    add_run(p, "Abstract", name=CFG["font_serif"],
            size_pt=CFG["size_abstract"], bold=True, italic=True, color=(0, 0, 0))
    add_run(p, "—", name=CFG["font_serif"],
            size_pt=CFG["size_abstract"], bold=True, color=(0, 0, 0))
    add_runs_with_inline(p, abstract, base_font=CFG["font_serif"],
                         base_size=CFG["size_abstract"], base_bold=True)
...
def add_keywords(doc, data):
    ...
    set_para_spacing(p, before_pt=4, after_pt=8,
                     alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
                     first_line_tw=274)
    add_run(p, "Keywords", name=CFG["font_serif"],
            size_pt=CFG["size_keywords"], bold=True, italic=True, color=(0, 0, 0))
    add_run(p, "—", name=CFG["font_serif"],
            size_pt=CFG["size_keywords"], bold=True, color=(0, 0, 0))
    add_run(p, kw_str, name=CFG["font_serif"],
            size_pt=CFG["size_keywords"], bold=True, italic=True, color=(0, 0, 0))
...
def add_section_heading(doc, title, num):
    ...
    set_para_spacing(p, before_pt=8, after_pt=4,
                     ...
                     first_line_tw=0)
    add_run(p, label, name=CFG["font_serif"],
            size_pt=CFG["size_heading1"], color=(0, 0, 0))
    reset_first_para()
...
def add_subsection_heading(doc, title, sub_idx):
    ...
    set_para_spacing(p, before_pt=6, after_pt=3,
                     alignment=WD_ALIGN_PARAGRAPH.LEFT, keep_next=True,
                     first_line_tw=0)
    add_run(p, label, name=CFG["font_serif"],
            size_pt=CFG["size_heading2"], italic=True, color=(0, 0, 0))
    reset_first_para()
...
def add_body_text(doc, text):
    ...
    set_para_spacing(p, before_pt=0, after_pt=6, line_pt=11.4,
                     alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
                     first_line_tw=first)
    add_runs_with_inline(p, text, base_font=CFG["font_serif"],
...
def add_figure(doc, fig):
    ...
    remove_numpr(img_p)
    set_para_spacing(img_p, before_pt=4, after_pt=2,
                     alignment=WD_ALIGN_PARAGRAPH.CENTER, first_line_tw=0)
    ...
    if not inserted:
        ...
        add_run(img_p, placeholder, name=CFG["font_serif"],
                size_pt=CFG["size_caption"], italic=True,
                color=(0xFF, 0x00, 0x00))

    cap = _new_para(doc, "figurecaption")
    remove_numpr(cap)
    set_para_spacing(cap, before_pt=4, after_pt=10,
                     alignment=WD_ALIGN_PARAGRAPH.CENTER,
                     first_line_tw=0, left_tw=0)
    num = fig.get("ImageNumber", "?")
    title = fig.get("Title", "Figure")
    add_run(cap, f"Fig. {num}.", name=CFG["font_serif"],
            size_pt=CFG["size_caption"], color=(0, 0, 0))
    add_run(cap, " " + title + ".", name=CFG["font_serif"],
            size_pt=CFG["size_caption"], color=(0, 0, 0))
    reset_first_para()
...
def add_formula(doc, fm):
    ...
    p = doc.add_paragraph()
    set_para_spacing(p, before_pt=4, after_pt=4, line_pt=12,
                     alignment=WD_ALIGN_PARAGRAPH.LEFT, first_line_tw=0)
    pf = p.paragraph_format
    ...
    add_run(p, formula_text, name=CFG["font_math"],
            size_pt=CFG["size_formula"], italic=True, color=(0, 0, 0))
    add_run(p, "\t(" + num + ")", name=CFG["font_serif"],
            size_pt=CFG["size_formula"], color=(0, 0, 0))
    reset_first_para()
...
def add_table(doc, tb):
    ...
    remove_numpr(cap)
    set_para_spacing(cap, before_pt=8, after_pt=4,
                     alignment=WD_ALIGN_PARAGRAPH.CENTER,
                     first_line_tw=0, left_tw=0, keep_next=True)
    add_run(cap, f"TABLE {num}", name=CFG["font_serif"],
            size_pt=CFG["size_table"], color=(0, 0, 0))
    add_run(cap, "\n", name=CFG["font_serif"], size_pt=CFG["size_table"])
    add_runs_with_inline(cap, title.upper(), base_font=CFG["font_serif"],
                         base_size=CFG["size_table"], base_italic=True)
    ...
    for j, h in enumerate(headers):
        ...
        set_para_spacing(cp, before_pt=2, after_pt=2,
                         alignment=WD_ALIGN_PARAGRAPH.CENTER, first_line_tw=0)
        add_runs_with_inline(cp, str(h), base_font=CFG["font_serif"],
                             base_size=CFG["size_table"], base_bold=True)
    ...
    for i, row in enumerate(rows):
        ...
        for j in range(n_cols):
            ...
            cell.text = ""
            cp = cell.paragraphs[0]
            set_para_spacing(cp, before_pt=1, after_pt=1,
                             alignment=WD_ALIGN_PARAGRAPH.CENTER,
                             first_line_tw=0)
            add_runs_with_inline(cp, str(val), base_font=CFG["font_serif"],
                                 base_size=CFG["size_table"])
            _set_cell_three_line(cell, top=None,
                                 bottom=12 if is_last else None)
    reset_first_para()
...
def process_section(doc, sec, num):
    ...
    sub_keys = sorted([k for k in sec.keys()
                       if k not in ("title", "content") and isinstance(sec[k], dict)])
    sub_idx = 0
    for k in sub_keys:
        sub_idx += 1
        sub = sec[k]
...
def add_references(doc, data):
    ...
    title = "References"
    items = refs.get("content", []) if isinstance(refs, dict) else (refs if isinstance(refs, list) else [])

    head = _new_para(doc, "Heading1")
    remove_numpr(head)
    set_para_spacing(head, before_pt=8, after_pt=4,
                     alignment=WD_ALIGN_PARAGRAPH.CENTER, keep_next=True,
                     first_line_tw=0)
    add_run(head, title, name=CFG["font_serif"],
            size_pt=CFG["size_heading1"], color=(0, 0, 0))
    ...
    for i, ref in enumerate(items, 1):
        ...
        remove_numpr(p)
        set_para_spacing(p, before_pt=0, after_pt=2.5, line_pt=9,
                         alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
                         left_tw=288, hanging_tw=288)
        add_run(p, f"[{i}] ", name=CFG["font_serif"],
                size_pt=CFG["size_ref"], color=(0, 0, 0))
        add_runs_with_inline(p, str(ref), base_font=CFG["font_serif"],
...
def generate():
    ...
    insert_section_break(doc,
                         CFG["title_top_tw"], CFG["title_bottom_tw"],
                         CFG["title_left_tw"], CFG["title_right_tw"],
                         cols=1, col_space_tw=720,
                         title_pg=True, footer_first_rid="rId8")

    # ── S2: author block (3 kolom) — gabungan author/affiliation
    add_authors(doc, data)
    insert_section_break(doc,
                         CFG["author_top_tw"], CFG["body_bottom_tw"],
                         CFG["author_left_tw"], CFG["author_right_tw"],
                         cols=3, col_space_tw=CFG["author_col_space_tw"])
    ...
    insert_section_break(doc,
                         CFG["body_top_tw"], CFG["body_bottom_tw"],
                         CFG["body_left_tw"], CFG["body_right_tw"],
                         cols=2, col_space_tw=CFG["col_space_tw"])
