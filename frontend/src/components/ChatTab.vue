<template>
 ...
 class="opacity-0 group-hover:opacity-100 text-[var(--text-muted)] hover:text-[var(--text-strong)] text-xs min-h-[44px] min-w-[44px] flex items-center justify-center active:scale-95 "
 title="Rename"
 ...
 class="opacity-0 group-hover:opacity-100 text-[var(--text-muted)] hover:text-red-500 dark:text-red-400 text-xs min-h-[44px] min-w-[44px] flex items-center justify-center active:scale-95 "
 title="Delete"
 ...
 class="flex items-center gap-1 px-2 py-1 min-h-[36px] text-[11px] font-medium text-[var(--text-muted)] hover:text-navy-600 dark:hover:text-cream-300 rounded-md hover:bg-navy-50 dark:hover:bg-ash-700 disabled:opacity-40 transition active:scale-95"
 ...
 class="flex items-center gap-1.5 px-2.5 py-1 min-h-[36px] text-[11px] font-medium text-[var(--text-muted)] hover:text-[#238f7f] dark:hover:text-[#4eb2a3] rounded-md hover:bg-green-50 dark:hover:bg-green-900/20 disabled:opacity-40 transition active:scale-95"
 ...
 <button @click="openPreview" class="underline font-semibold hover:text-amber-700 dark:hover:text-amber-100 active:scale-95 ">Buka Preview</button>
 </span>
 </div>
 <div class="flex items-center gap-1.5 mt-1.5">
 <button
 @click="paperStore.acceptAllProposals()"
 class="flex-1 px-2.5 py-1 min-h-[44px] text-[11px] font-semibold rounded-md bg-emerald-600 hover:bg-emerald-700 text-white active:scale-95 "
 >✓ Terima semua</button>
 <button
 @click="paperStore.rejectAllProposals()"
 class="flex-1 px-2.5 py-1 min-h-[44px] text-[11px] font-semibold rounded-md bg-rose-100 hover:bg-rose-200 dark:bg-rose-900/40 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 active:scale-95 "
 >✕ Tolak semua</button>
 </div>
 </div>
 ...
 class="shrink-0 px-2.5 py-1 min-h-[36px] text-[11px] font-semibold rounded-md bg-red-600 hover:bg-red-700 text-white transition active:scale-95 "
 >↻ Retry</button>
 <button
 @click="lastError = null"
 class="shrink-0 text-red-500 hover:text-red-700 dark:text-red-300 dark:hover:text-red-100 min-h-[36px] min-w-[36px] flex items-center justify-center active:scale-95 "
 title="Tutup"
 >✕</button>
 </div>
 ...
 v-if="f.canRetry"
 @click="retryUpload(i)"
 class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200 ml-1 min-h-[44px] min-w-[44px] active:scale-95 "
 title="Retry upload"
 ...
 class="text-ink-500 dark:text-ink-300 hover:text-rose-500 ml-1 min-h-[44px] min-w-[44px] active:scale-95 "
 title="Remove"
 ...
 class="shrink-0 min-h-[44px] min-w-[44px] px-2 text-base text-ink-700 dark:text-ink-200 hover:bg-cream-200 dark:hover:bg-ash-600 rounded-lg transition active:scale-95 disabled:opacity-40"
 ...
 @click="handleStop"
 class="shrink-0 min-h-[44px] min-w-[44px] bg-red-600 hover:bg-red-700 text-white rounded-lg transition active:scale-95 flex items-center justify-center"
 ...
 class="shrink-0 min-h-[44px] min-w-[44px] bg-navy-700 hover:bg-navy-800 dark:bg-cream-200 dark:hover:bg-cream-100 text-cream-50 dark:text-ash-900 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition active:scale-95 flex items-center justify-center"
 title="Send"
 ...
 <template #actions>
 <button @click="deleteTarget = null" class="px-3 py-1.5 min-h-[44px] text-sm rounded-lg border border-cream-300 dark:border-ash-700 hover:bg-cream-100 dark:hover:bg-ash-700 active:scale-95 ">Cancel</button>
 <button @click="doDeleteChat" class="px-3 py-1.5 min-h-[44px] text-sm rounded-lg bg-red-600 hover:bg-red-700 text-white active:scale-95 ">Delete</button>
 </template>
 </AppDialog>
 ...
 <template #actions>
 <button @click="exportDraftOpen = false" class="px-3 py-1.5 min-h-[44px] text-sm rounded-lg border border-cream-300 dark:border-ash-700 hover:bg-cream-100 dark:hover:bg-ash-700 active:scale-95 ">Cancel</button>
 <button
 @click="doExportDraft"
 :disabled="exportDraftSaving || !exportDraftName.trim()"
 class="px-3 py-1.5 min-h-[44px] text-sm rounded-lg bg-[#238f7f] hover:bg-[#1d7a6d] text-white disabled:opacity-50 active:scale-95 "
 >
...
const FUN_WAIT_MESSAGES_EN = [
 'Thinking... (brewing ideas)',
 'Thinking... (crafting words)',
 'Thinking... (refining arguments)',
 'Thinking... (checking details)',
 'Thinking... (distilling insights)',
 'Thinking... (final touches)',
 'Almost there...',
]
const FUN_WAIT_MESSAGES = computed(() =>
 paperStore.paper.language === 'en' ? FUN_WAIT_MESSAGES_EN : FUN_WAIT_MESSAGES_ID
)
const streamingFunMessage = ref(FUN_WAIT_MESSAGES_ID[0])
let _funMsgIndex = 0
...
function startStreamingTimer(): void {
 ...
 _funMsgIndex = 0
 streamingFunMessage.value = FUN_WAIT_MESSAGES.value[0]
 ...
 _funMsgIndex = (_funMsgIndex + 1) % FUN_WAIT_MESSAGES.value.length
 streamingFunMessage.value = FUN_WAIT_MESSAGES.value[_funMsgIndex]
 }
 }
...
async function retryLastMessage(): Promise<void> {
 ...
 showToast(paperStore.paper.language === 'en' ? 'No message to retry' : 'Tidak ada pesan untuk di-retry', 'warning')
 return
 }
 
 lastError.value = null
 showToast(paperStore.paper.language === 'en' ? 'Resending message...' : 'Mengirim ulang pesan...', 'info')
 
 try {
 await chatStore.sendMessage(lastUserMessage.value)
 } catch (e: any) {
 lastError.value = e.message || (paperStore.paper.language === 'en' ? 'Failed to send message' : 'Gagal mengirim pesan')
 showToast(paperStore.paper.language === 'en' ? 'Failed to resend message' : 'Gagal mengirim ulang pesan', 'error')
 }
}
...
const statusMessage = computed(() => {
 ...
 const isEn = paperStore.paper.language === 'en'

 // Phase-based status (most accurate during streaming)
 const phase = streamPhase.value
 if (phase === 'sending') return isEn ? 'Sending message...' : 'Mengirimkan pesan...'
 if (phase === 'thinking') return isEn ? 'Thinking...' : 'Masih berpikir...'
 if (phase === 'composing') return isEn ? 'Composing answer...' : 'Menyusun jawaban...'
 if (phase === 'streaming') return isEn ? 'Typing...' : 'Mengetik...'

 // Fallback: legacy detection via message content
 const msg = messages.value[messages.value.length - 1]
 if (msg?.role !== 'assistant') return isEn ? 'Connecting...' : 'Menghubungkan...'
 ...
 const toolNamesId: Record<string, string> = {
 ...
 'SearchLiterature': '📚 Searching literature',
 'SearchArxiv': '📚 Searching arXiv',
 'SearchSemanticScholar': '🎓 Searching Semantic Scholar',
 'RunSLR': '🔬 Systematic Literature Review',
 'SearchPapers': '📄 Searching papers',
 'GenerateFullPaper': '📝 Generating full paper',
 'SaveMemory': '💾 Saving note',
 'AskQuestions': '❓ Preparing questions',
 'StartWorkflow': '⚙️ Setting up initial data',
 'SaveWorkflowAnswers': '💾 Saving answers',
 'ProposeSection': '✍️ Writing section',
 'ProposeTitle': '📌 Writing title',
 'ProposeAbstract': '📄 Writing abstract',
 'ProposeKeywords': '🏷️ Writing keywords',
 'ProposeReference': '📖 Adding reference',
 'GenerateImage': '🖼️ Generating image',
 'GenerateChart': '📊 Generating chart',
 'CreateChart': '📊 Creating chart',
 'ReviewLargeFile': '📋 Reviewing file',
 'ClassifyFile': '📂 Processing file',
 'Paraphrase': '✏️ Paraphrasing text',
 'FixGrammar': '🔧 Fixing grammar',
 'Translate': '🌐 Translating',
 'RequestExportDocx': '📥 Exporting DOCX',
 'ReviewPaper': '🔍 Reviewing paper',
 }
 const toolNames = isEn ? toolNamesEn : toolNamesId
 return toolNames[lastTool.name] || (isEn ? 'Processing...' : 'Memproses...')
 }
 }

 // Check for thinking vs typing
 if (msg.thinking && !msg.content) return isEn ? 'Thinking...' : 'Masih berpikir...'
 if (msg.content) return isEn ? 'Typing...' : 'Mengetik...'

 return isEn ? 'Sending message...' : 'Mengirimkan pesan...'
})
...
function conversationTitle(title?: string | null): string {
 ...
 ? (paperStore.paper.language === 'en' ? 'Untitled chat' : 'Chat tanpa judul') 
 : normalized
}
...
async function handleSend(): Promise<void> {
 ...
 attachedFiles.value.forEach(f => {
 if ((f as any).uploading) {
 (f as any).uploading = false
 }
 })
 ...
 const MAX_FILE_CHARS = 200000  // raised from 30K — user wants full content; backend handles per-file at 100K+
 const blocks = fileEntries.map(e => {
   let content = e.text || '(ekstrak teks gagal / kosong)'
   if (content.length > MAX_FILE_CHARS) {
     content = content.slice(0, MAX_FILE_CHARS) + `\n\n[... teks dipotong di ${MAX_FILE_CHARS} karakter. Konten lengkap tersedia di sistem referensi file.]`
   }
 ...
 attachedFiles.value.forEach(f => {
 ;(f as any).uploading = false
 ;(f as any).uploadError = ''
 ;(f as any).canRetry = false
 })
 return
 }
 ...
 // IMPORTANT: Use Object.assign instead of {...f} spread to preserve File binary data
 attachedFiles.value.forEach(f => {
 ;(f as any).uploading = false
 ;(f as any).uploadError = msg
 ;(f as any).canRetry = status !== 401 // Don't allow retry on auth errors
 })
...
function addAttachedFiles(files: File[]): void {
 ...
 const imageFiles = files.filter(f => /^image\/(jpeg|jpg|png|gif|webp|bmp)$/i.test(f.type))
 const docFiles = files.filter(f => /\.(pdf|docx|doc|txt|md)$/i.test(f.name))
 ...
 attachWarning.value = 'File tidak didukung. Gunakan PDF, DOCX, DOC, TXT, MD, atau gambar (JPG, PNG, GIF, WebP).'
 if (_attachWarningTimer) clearTimeout(_attachWarningTimer)
 _attachWarningTimer = window.setTimeout(() => { attachWarning.value = '' }, 5000)
 }
 ...
 const merged = [...attachedFiles.value, ...docFiles].slice(0, 50)
 if (merged.length === 50 && (attachedFiles.value.length + docFiles.length) > 50) {
 attachWarning.value = 'Maksimal 50 file. Sisanya diabaikan.'
 }
...
async function retryUpload(i: number): Promise<void> {
 ...
 // Clear error state and mark as uploading (mutate in-place to preserve File data)
 ;(file as any).uploading = true
 ;(file as any).uploadError = undefined
 ;(file as any).canRetry = false
 ...
 ;(file as any).uploadError = msg
 ;(file as any).canRetry = status !== 401
