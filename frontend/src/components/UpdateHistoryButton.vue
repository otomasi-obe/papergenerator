<template>
  ...
  <div class="relative" ref="rootRef">
    <button
      @click="togglePanel"
      class="flex items-center gap-1.5 px-3 py-1.5 min-h-[36px] rounded-lg bg-cream-100 dark:bg-ash-700 border border-cream-300 dark:border-ash-600 text-ink-700 dark:text-[#fef9c3] hover:bg-cream-200 dark:hover:bg-ash-700 active:scale-95 transition text-xs font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/50"
      ...
      >
        ...
        <div class="flex items-center justify-between px-4 py-2.5 bg-cream-50 dark:bg-ash-700 border-b border-cream-200 dark:border-ash-600 shrink-0">
          <div class="flex items-center gap-2">
            ...
            <span class="text-[11px] text-ink-600 dark:text-[#fef08a]">{{ versions.length }} versi</span>
          </div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto p-2.5 space-y-2">
          <div v-for="v in versions" :key="v.version" class="bg-cream-50 dark:bg-ash-700/50 rounded-lg border border-cream-200 dark:border-ash-600 p-2.5">
            <div class="flex items-start justify-between gap-2 mb-1.5">
              ...
              <span class="text-[10px] text-ink-600 dark:text-[#fef08a] whitespace-nowrap">{{ formatDate(v.released) }}</span>
            </div>
            <ul v-if="v.changes?.length" class="space-y-1 pl-1">
              <li v-for="(c, i) in v.changes" :key="i" class="flex items-start gap-2 text-xs leading-relaxed text-ink-800 dark:text-[#fef9c3]">
          ...
          <div v-if="!versions.length" class="text-center py-6 text-ink-600 dark:text-[#fef08a] text-sm">Belum ada riwayat update</div>
        </div>
        ...
        <div class="px-3 py-2 bg-cream-50 dark:bg-ash-700 border-t border-cream-200 dark:border-ash-600 shrink-0 flex items-center justify-between">
          <span class="text-[10px] text-ink-600 dark:text-[#fef08a]">Dicek: {{ lastChecked }}</span>
