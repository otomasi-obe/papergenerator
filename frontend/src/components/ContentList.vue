<template>
  <draggable ref="listRoot" :list="items" :item-key="stableKey" animation="150" handle=".content-drag" class="space-y-1.5"
    ...
    <div class="relative">
      <!-- Box (utama) -->
      <div class="bg-cream-50 dark:bg-ash-700 border border-cream-200 dark:border-ash-500 rounded-lg p-3 relative group transition-colors"
           :data-content-box="idx">

        <!-- Item header -->
        <div class="flex items-center justify-between mb-1.5">
          <div class="flex items-center gap-1.5">
             <span class="content-drag cursor-grab active:cursor-grabbing text-ash-400 dark:text-ash-300 hover:text-navy-500 dark:hover:text-cream-300 select-none text-base leading-none px-0.5" role="button" aria-label="Drag to reorder">⠿</span>
        ...
        <!-- TEXT -->
                <template v-if="item.id === 'text'">
                  <textarea :value="item.text"
                    ...
                    v-autosize
                    rows="2"
                     class="content-textarea-auto w-full px-2.5 py-2 border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 text-navy-900 dark:text-ink-50 dark:placeholder-ash-400 rounded text-sm focus:ring-2 focus:ring-cream-200 focus:border-navy-400 outline-none break-words resize-none overflow-hidden"
        ...
        <template v-else-if="item.id === 'gambar'">
          <div class="space-y-2">
             <input v-model="item.Title" class="w-full px-2.5 py-1.5 border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 text-navy-900 dark:text-ink-50 dark:placeholder-ash-400 rounded text-sm outline-none focus:border-navy-400"
              placeholder="Image Title / Caption" />

            <!-- Thumbnail (always rendered when there's a path or live preview from current job) -->
            <div v-if="item.Path" class="rounded border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 overflow-hidden flex items-center justify-center" style="max-height:280px">
            ...
            <div class="flex items-center gap-2 flex-wrap">
               <label class="px-2.5 py-1.5 bg-cream-100 dark:bg-ash-600 hover:bg-cream-200 dark:hover:bg-ash-500 text-navy-700 dark:text-ash-100 rounded text-xs cursor-pointer flex items-center gap-1 font-medium focus-visible:ring-2 focus-visible:ring-[#238f7f]/30">
              ...
              <button @click="toggleGallery(item)" type="button"
                class="px-2.5 py-1.5 bg-cream-100 dark:bg-ash-600 hover:bg-cream-200 dark:hover:bg-ash-500 text-navy-700 dark:text-ash-100 rounded text-xs font-medium flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-[#238f7f]/30">
            ...
            <div v-show="galleryOpen[stableKey(item)]" class="rounded border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 p-2">
              <div v-if="!store.figureSources.length" class="text-[11px] text-ink-500 dark:text-ash-300 px-1 py-2 text-center">
                ...
                <button v-for="src in store.figureSources" :key="src.filename"
                  ...
                  :class="['group/src relative rounded border overflow-hidden text-left transition-all',
                    basename(item.Path) === src.filename
                      ...
                      : 'border-cream-300 dark:border-ash-500 hover:border-navy-400',
                  ...
                  :title="store.isSourceUsedByOther(src.filename, item)
                    ...
                    : src.label">
                  <div class="aspect-[4/3] bg-cream-100 dark:bg-ash-700 flex items-center justify-center">
                  ...
                  <span class="block text-[10px] text-ink-700 dark:text-ash-300 px-1 py-0.5 truncate">{{ src.label }}</span>
                </button>
              </div>
            </div>
            ...
            <div v-show="promptOpen[stableKey(item)]" class="space-y-1.5">
              <textarea :value="item.Prompt || ''"
                ...
                v-autosize
                rows="2"
                 class="content-textarea-auto w-full px-2.5 py-1.5 border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 rounded text-xs outline-none focus:border-navy-400 text-navy-700 dark:text-ash-100 dark:placeholder-ash-400 resize-none overflow-hidden"
          ...
          <div class="space-y-2">
            ...
            <div class="overflow-x-auto">
                ...
                <thead>
                  <tr>
                    <th v-for="(h, ci) in item.Headers" :key="ci"
                      class="border border-cream-200 dark:border-ash-500 bg-cream-100 dark:bg-ash-600 p-0 relative">
                      <input :value="h" @input="item.Headers[ci] = ($event.target as HTMLInputElement).value"
                        class="w-full px-2 py-1.5 text-xs font-semibold bg-transparent text-navy-900 dark:text-ink-50 outline-none text-center" />
                    ...
                    <th class="w-8">
                       <button @click="preserveScroll(() => store.addTableCol(item))"
                        class="text-navy-400 dark:text-ash-300 hover:text-navy-700 dark:hover:text-ash-50 text-xs focus-visible:ring-2 focus-visible:ring-[#238f7f]/30">+</button>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(row, ri) in item.Rows" :key="ri">
                    <td v-for="(cell, ci) in row" :key="ci" class="border border-cream-200 dark:border-ash-500 p-0">
                       <input :value="cell" @input="item.Rows[ri][ci] = ($event.target as HTMLInputElement).value"
                        class="w-full px-2 py-1 text-xs bg-transparent text-navy-900 dark:text-ink-50 outline-none" />
                    </td>
             ...
             <button @click="preserveScroll(() => store.addTableRow(item))"
              class="text-xs text-navy-600 dark:text-ash-100 hover:text-navy-800 dark:hover:text-ash-50 focus-visible:ring-2 focus-visible:ring-[#238f7f]/30">+ Add Row</button>
          </div>
        </template>

        <!-- FORMULA / RUMUS -->
                <template v-else-if="item.id === 'rumus'">
                   <input v-model="item.latex" class="w-full px-2.5 py-1.5 border border-cream-200 dark:border-ash-500 bg-cream-50 dark:bg-ash-700 text-navy-900 dark:text-ink-50 dark:placeholder-ash-400 rounded text-sm font-mono outline-none focus:border-navy-400"
                    placeholder="LaTeX formula, e.g. T_{total} \\approx \\max(T_{cap}, T_{inf}, T_{modbus})" />
                   <div v-if="item.latex" class="mt-1.5 text-center text-sm bg-cream-100 dark:bg-ash-600 dark:text-ash-100 px-2 py-2 rounded" v-html="renderFormula(item.latex)"></div>
                </template>
      </div>
      ...
      <div class="absolute right-2 bottom-0 translate-y-1/2 z-20" v-click-outside-content="() => closeInsert(idx)">
        <button type="button"
          ...
          :class="['w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold transition-all',
                   'bg-cream-100 dark:bg-ash-600 border border-cream-200 dark:border-ash-500 text-navy-500 dark:text-ash-100',
        ...
        <div v-if="insertOpen === idx"
             class="absolute right-0 bottom-full mb-1 bg-cream-50 dark:bg-ash-700 border border-cream-200 dark:border-ash-500 rounded-lg shadow-lg p-1 flex flex-col min-w-[140px] z-30">
          <button type="button" @click="insertAfter(idx, 'text')"
            class="text-left px-2.5 py-1 text-xs rounded hover:bg-cream-200 dark:hover:bg-ash-600 text-ink-900 dark:text-ink-50">📝 Text</button>
          <button type="button" @click="insertAfter(idx, 'gambar')"
            class="text-left px-2.5 py-1 text-xs rounded hover:bg-cream-200 dark:hover:bg-ash-600 text-ink-900 dark:text-ink-50">🖼 Image</button>
          <button type="button" @click="insertAfter(idx, 'tabel')"
            class="text-left px-2.5 py-1 text-xs rounded hover:bg-cream-200 dark:hover:bg-ash-600 text-ink-900 dark:text-ink-50">📊 Table</button>
          <button type="button" @click="insertAfter(idx, 'rumus')"
            class="text-left px-2.5 py-1 text-xs rounded hover:bg-cream-200 dark:hover:bg-ash-600 text-ink-900 dark:text-ink-50">∑ Formula</button>
        </div>
      </div>
    </div>
    </template>
...
function badgeClass(id: string): string {
  const map: Record<string, string> = {
     text: 'border-cream-300 text-navy-500 dark:text-ash-300',
     ...
     rumus: 'border-cream-400 text-navy-600 dark:text-ash-300'
  }
