<template>
  <div class="p-6 space-y-6">
    <!-- Title -->
    <div>
      <label class="block text-sm font-semibold text-ink-900 dark:text-ink-50 mb-1">Paper Title</label>
    ...
    <div>
      ...
      <div v-for="(author, index) in store.paper.authors" :key="index"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-ink-600 dark:text-anthracite-200 mb-1">Name</label>
            <input v-model="author.name" placeholder="Full Name" class="w-full px-3 py-1.5 border border-ivory-300 dark:border-anthracite-500 bg-white dark:bg-anthracite-800 text-ink-900 dark:text-anthracite-50 placeholder-ivory-500 dark:placeholder-anthracite-200 rounded text-sm focus:ring-2 focus:ring-ivory-300 dark:focus:ring-anthracite-500 outline-none" />
          </div>
          <div>
            <label class="block text-xs text-ink-600 dark:text-anthracite-200 mb-1">Email</label>
          ...
          <div>
            <label class="block text-xs text-ink-600 dark:text-anthracite-200 mb-1">Affiliation</label>
            <input v-model="author.affiliation" placeholder="University / Institute"
              class="w-full px-3 py-1.5 border border-ivory-300 dark:border-anthracite-500 bg-white dark:bg-anthracite-800 text-ink-900 dark:text-anthracite-50 placeholder-ivory-500 dark:placeholder-anthracite-200 rounded text-sm focus:ring-2 focus:ring-ivory-300 dark:focus:ring-anthracite-500 outline-none" />
          </div>
          <div>
            <label class="block text-xs text-ink-600 dark:text-anthracite-200 mb-1">Location</label>
    ...
    <!-- Keywords -->
    <div>
      <label class="block text-sm font-semibold text-ink-900 dark:text-ink-50 mb-1">Keywords</label>
